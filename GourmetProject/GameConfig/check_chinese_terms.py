...
SKIP_FILES = {
    ...
    PROJECT_ROOT / "docs" / "中文术语界面与图片审计.md",
...
RULES = (
    ...
    ("食物旧简称", re.compile(r"(?:逐道|每道|几道|多格|选中的|新的|能放下的)菜|一菜一|整菜技能|菜id")),
    ("玩家文案中的 Boss", re.compile(r"Boss\s*(?:节点|行动|食物|挑战|胜利|奖励|周|机制|修正|标记|随机|配置|完成|情况)|(?:最终|本周|普通)\s*Boss", re.IGNORECASE)),
    ("物品类型旧简称", re.compile(r"被动\s*/\s*主动|主动\s*/\s*被动")),
    ("数字与单位之间缺少空格", re.compile(r"(?<![A-Za-z0-9_.])\d+(?:\.\d+)?(?:金币|个食物|次营业|颗红心|次行动|个行动|个装饰品|个消耗品)")),
    ("倍率符号格式", re.compile(r"倍率\s*[xX*]\s*\d")),
