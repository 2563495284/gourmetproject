...
> 当前配置已拆为 `TbPassiveItem` 与 `TbActiveItem`，两张表均在运行时启用。被动道具以 ID 绑定模型并自动触发；主动道具以 `effectType + targetKind` 描述玩家主动使用的效果。
> 被动表只保留目标分、菜品、被动道具与碎片四类常驻隐藏分修正；主动道具没有隐藏分，金币隐藏分也不接受被动道具修正。
>
...
| 隐藏分 | `hiddenRange`，并可修正目标分/菜品/被动/碎片 | 不配置，抽取只按 `baseWeight` | 被动池专用 |
...
主动 `effectType` 常量集中在 `Game/Meta/Items/ItemEffectTypes.cs`；被动道具不填写 `effectType`，每个启用 ID 必须有且只有一个对应的 `PassiveItemModel`。
...
1. ✅ **配置**：`item.xlsx` 已拆成 `被动道具` / `主动道具` 两张 sheet；`TbPassiveItem` 有 `hiddenRange`，`TbActiveItem` 有 `targetKind` / `targetCount` 且不使用隐藏分；生成 `PassiveItem.cs` / `ActiveItem.cs`。
