...
> **⚠️ 当前配置主动表为空（2026-07 改造）**
> 依《道具设计》sheet「全是被动道具」的意图，原本被误标为主动的 29 条已全部迁入 `TbPassiveItem`，
> 并改为「获得瞬间结算一次、之后作为被动常驻保留」的 `effectType` 族（由 `ItemEffectTypes.IsOnAcquireEffect` + `PassiveOnAcquireEffects` 派发，不再单独配 `triggerTiming`）。**主动道具子系统（消耗槽、`ActiveItemEffectRegistry`、
> `BattleForm` 主动使用、`ItemActiveUsage`）保留代码但当前为空运行**（配置表无主动项，主动池抽取恒为空）。
> 引用「主动道具」的效果（`GrantRandomActive`/`ChooseOneActive`/`ExtraActiveSlot`/`GoldOnActiveUse`/`BlockActive`）随之失效或惰性，标 `TODO(passive-item)`。
> 本次实现：纯数值/资源类被动效果已接线生效（金币/利息/目标分/奖励可选数量/周末结算/计数类等）；
> 涉及 UI / 选目标 / 战斗循环内 / 事件概率权重 / 时间轴等复杂交互的效果统一以 `// TODO(passive-item)` 打桩，数值判定已集中在 `ItemRuntime`。
> 下文「主动道具」章节描述的是保留的历史架构，非当前启用形态。
>
> 面向策划 + 程序。定义「被动道具」与「主动道具」的概念边界、配置语法、运行时架构与落地方式。
>
> 配置源：`GameConfig/Datas/item.xlsx`（sheet `被动道具` → `TbPassiveItem`，sheet `主动道具` → `TbActiveItem`）+ `__enums_item.xlsx`（枚举）。字段速查见 [配置表说明.md](../配置表说明.md) 5.3 节。
...
配置已按机制物理拆为 `TbPassiveItem` / `TbActiveItem`。被动表保留 `hiddenRange`，主动表保留 `targetKind` / `targetCount` 且不配置 `hiddenRange`；主动道具是一个以「目标类型」为核心、跨情境可用的独立子系统。
...
| --- | --- | --- |
| `DiningTableDish` | 餐桌上的菜 | 仅战斗；`AddFlavor` 额外限制为 Food 主界面且目标必须有菜谱来源 |
...
调味与铺台小票是效果级特例：两者都只在未结算的 Food 战斗主界面开放。
...
| --- | --- | --- | --- |
| `hiddenRange` | ✅ | — | 被动道具按隐藏分进池 |
| `targetKind` | — | 见 4.1 | 主动道具目标类型，进 `__enums_item.xlsx` |
| `targetCount` | — | 见 4.2 | 选几个目标 |
| `effectType` | 被动词表 | 主动词表 | **两套词表 + 填表校验** |
| `effectValue` / `effectParam` | ✅ | ✅ | 操作参数 |
| `quality` / `baseWeight` / `specialTags` | ✅ | ✅ | 两表各自配置 |

**词表校验**：`effectType` 按表分成被动/主动两份合法值集合，填错报错。常量集中在 `Game/Meta/Items/ItemEffectTypes.cs`，建议按表分区并加校验入口。
...
同一操作在不同情境的落地由 `IActiveUseContext` 区分（如 `AddScore`：`DiningTableDish` 目标改本局分、`RecipeDish` 目标永久改菜谱）。

`AddFlavor` 选择桌上菜后，同时修改该 `DishInstance` 与其 `SourceDishIndex` 指向的 `RecipeBookSlot`；
`AddMaterial` 同时修改当前餐桌与当前 Run 的材质覆盖。主动道具只修改内存并消费道具，不直接落盘；动画不承担数据修改，最终存档遵循既有节点。
...
1. ✅ **配置**：`item.xlsx` 已拆成 `被动道具` / `主动道具` 两张 sheet；`TbPassiveItem` 有 `hiddenRange`，`TbActiveItem` 有 `targetKind` / `targetCount` 且当前为空；已跑 `bash GameConfig/gen.sh` 生成 `PassiveItem.cs` / `ActiveItem.cs`。（`effectType` 按表拆词表 + 校验暂缓）
...
3. 🚧 **主动子系统**：已完成 C# 核心——`IActiveUseContext` + `ActiveTarget`、`BattleUseContext`、`ItemActiveUsage`（targetKind→情境/选目标推导）、`ActiveItemEffectRegistry.Apply(ctx,item,targets)` 情境无关派发（`TryUse` 保留为战斗便捷入口）。**待办**：主动表填数据后接 Meta（商店/地图/奖励）情境实现 + 选目标状态机/UI。
