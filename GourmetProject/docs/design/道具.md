...
- **砍掉 per-item `holdLimit` 的囤积语义**：清理当前「能囤 9 份却只显 2 槽」的双轨矛盾（`BattleWorldController.ActiveSlotCapacity=2` 写死 vs `TbItem.holdLimit`）。
- 槽满时再获得主动道具 → 折金币（复用 `ItemAcquireOutcome.ConvertedToGold`）。
- 槽位数在**所有情境**统一（战斗 / 商店 / 地图 / 奖励界面），不再只在战斗 UI 生效。

> 迁移说明：`holdLimit` 列保留物理存在但不再表达「同 id 囤积上限」；主动道具持有量由全局槽约束。
...
1. **配置**：`__enums_item.xlsx` 加 `ItemTargetKind` 枚举；`item.xlsx` 加 `targetKind` / `targetCount` 列；`effectType` 按 kind 拆词表 + 校验；跑 `bash GameConfig/gen.sh`。
2. **槽位**：全局消耗槽（基础 2 + `ExtraActiveSlot`）替换写死的 `ActiveSlotCapacity`；`AcquireItem` 主动分支改为「槽满折金币」。
3. **主动子系统**：`IActiveUseContext`（战斗 + Meta 两实现）；`ActiveItemEffectRegistry.Apply(ctx,item,targets)` 解耦；选目标状态机。
4. **随机**：`SeedDomains.Item` + `activeUseIndex`（存档字段）。
5. **持有视图**：`PassiveItems` / `ActiveItems` 暴露，收敛散落的 kind 过滤。
6. **测试**：主动道具跨情境使用、槽满折金币、随机生成可复现（对齐 `RngDeterminismTests` 风格）。
