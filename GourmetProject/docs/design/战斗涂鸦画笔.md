...
- 按钮：复用世界空间 `WorldButtonView`（与「吃!」「总览」一致），固定结构预置在场景。
...
flowchart LR
    ...
    BWC["BattleWorldController.Initialize\n进新战斗 -> Clear() + 复位"]

    Mouse --> Ctrl --> Stroke
    ClearBtn -->|"Clear()"| Stroke
    ToggleBtn -->|"SetVisible()"| Stroke
    BWC -->|"ConfigureDoodleHud()"| Ctrl
```
...
| `Assets/GameMain/Scripts/Game/Gameplay/Presentation/BattleSorting.cs` | 新增常量 `public const string Doodle = "Doodle";` |
| `Assets/GameMain/Scripts/Game/Gameplay/Presentation/WorldInput.cs` | 新增 `SecondaryHeld`、`SecondaryReleasedThisFrame` |
| `Assets/GameMain/Scripts/Game/Gameplay/Presentation/BattleDoodleController.cs` | 新建：涂鸦核心控制器 |
| `Assets/GameMain/Scripts/Game/Gameplay/Presentation/BattleWorldController.cs` | 新增 `_doodle` / `_clearDoodleButton` / `_toggleDoodleButton` 字段 + `ConfigureDoodleHud()` / `ToggleDoodleVisible()` |
| `Assets/GameMain/Scenes/Battle.unity` | `BattleSceneRoot/DoodleRoot` 挂控制器；`Menu` 下预置两个涂鸦按钮 |
...
### 4.4 HUD 接入（BattleWorldController）

两个按钮是**固定结构**（数量恒定），按 `presentation-prefab` 规则：预置在场景 + `[SerializeField]` 引用 + 运行时只 `Configure`，**不**在代码里 `Instantiate` 或摆位置。
...
`Initialize` 末尾调用 `ConfigureDoodleHud()`：

- 给两个按钮喂大小 / 文案 / 颜色 / 回调（位置来自场景）。
- 每次进入战斗（含再战）执行 `_doodle.Clear()` + `_doodle.SetVisible(true)` + 复位标签为「隐藏涂鸦」，实现「进新战斗自动清空」。

`ToggleDoodleVisible()` 翻转 `_doodle.IsVisible` 并把标签在「隐藏涂鸦 / 显示涂鸦」间切换。
...
└── BattleSceneRoot
    ├── BattleWorldController   (_doodle / _clearDoodleButton / _toggleDoodleButton 已接)
    ├── DoodleRoot              (BattleDoodleController, _camera → BattleCamera)
    │   └── Strokes             (运行时生成的笔迹挂这里)
    └── Menu
        ├── EatButton / OverviewButton ...
        ├── DoodleClearButton   (WorldButtonView, 世界坐标约 8.7, -4.95)
        └── DoodleToggleButton  (WorldButtonView, 世界坐标约 7.1, -4.95)
```

两个按钮位于底边最下方一排，在主动道具区（自 `-halfH + 1.3` 起向上排）之下，互不重叠。
...
| 调按钮位置 | 在 `Battle.unity` 移动 `Menu/DoodleClearButton`、`DoodleToggleButton` |
