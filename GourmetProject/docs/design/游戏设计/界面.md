...
```mermaid
flowchart TD
  ActionSelect["ActionSelect 行动 / 事件选择"] -->|"选食物 / Boss 行动"| Food["Food 经营挑战(世界)"]
...
| 周末星级评鉴（恶魔） | Boss 节点 | 标题 `周末星级评鉴 / 恶魔` + 美术图 + 技能描述框 + `节点行动` |
...
**`ShopForm` 已瘦身为商店购买区**：编辑食谱抽出为独立状态 `RecipeEditPanel`（第 4 节），`ShopForm` 负责食物 / 碎片包 / 被动 / 主动四区，以及独立的删除食物服务区与入口回调。
