...
| `Shown` | `Shop` / `Food` 态 | 完全展开 | 展示唯一菜谱；战斗态可点上菜 |
...
- 商店态 `RecipeView` 展示唯一菜谱（`已放/容量`）。
- 商店购买的食物直接加入该菜谱，不再选择目标菜谱，也不再出售空菜谱。
...
- **顶部提示**：删除食物至垃圾桶 / 拖拽移动。
- **中部菜谱区**：展示唯一菜谱（`_editBooksContainer` 铺一个 `RecipeEditBookView`），右上角容量 `已放/上限`，菜品格 2 列排布；菜品以「菜品 + 背景网格」表现占格特性（第 6 节）。
- **底部**：垃圾桶（`RecipeTrashDropZone`，拖入删除、花金币 `ShopService.DeleteDishCost`）+ `离开编辑` 按钮（回调 `onExit` → 返回商店态）。

### 4.2 交互

- **拖拽整理**：调整唯一菜谱内的菜品顺序（`ShopService.MoveDish`）。
