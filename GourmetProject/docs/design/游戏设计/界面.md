...
局外全流程与局内战斗共用同一套**常驻壳**，进入玩法后常驻壳**全程不隐藏**，只有**中部内容区**在 `GameplayView` 各状态之间切换：
...
flowchart TB
  subgraph shell [常驻壳 Shell - 进入玩法全程不隐藏]
    Left["左列: 周 / 金币 / 分数要求 / 查看菜谱 / 查看餐桌 / 设置"]
    Right["右列: 被动网格 + 主动槽"]
...
└─ HudFrame                       ← 进入玩法后常驻（仅开局前 / 结算返回菜单时整体隐藏）
   ...
   ├─ LeftColumn                  ← 常驻壳：左列信息 + 查看菜谱/餐桌 + 系统按钮
...
| 查看菜谱 | 按钮 | 打开中部只读菜谱页；再次点击或点返回回到原状态 | `_viewRecipeButton` |
...
BattleForm 不再保留底部固定菜谱卡。左列 `ViewRecipe` 是常驻入口，点击后切换到中部
`RecipeInspect`，由 `RecipeReadonlyBookView` 展示唯一菜谱；从战斗态打开时使用
`BattleSession` 的剩余菜品快照。
...
行动轴节点行动（参考 [行动轴事件页签.png](../原型图/行动轴事件页签.png)）：

| 卡面 | 说明 | 卡内展示 |
|---|---|---|
| 商店 | 节点商店 | 标题 `商店` + 美术图 + 底部 `节点行动` 标签 |
| 收取利息 | 利息节点 | 标题 `收取利息` + 效果描述 + `节点行动` |
| 周末盛宴（恶魔） | Boss 节点 | 标题 `周末盛宴 / 恶魔` + 美术图 + 技能描述框 + `节点行动` |
...
参考 [商店.png](../原型图/商店.png)。左右壳与行动轴常驻，中部为商品购买区和删除食物服务区；菜谱从左列 `ViewRecipe` 进入。对应 `ShopForm`（`Assets/GameMain/Scripts/Game/UI/Meta/ShopForm.cs`）。

**`ShopForm` 已瘦身为商店购买区**：编辑菜谱抽出为独立状态 `RecipeEditPanel`（第 4 节），`ShopForm` 负责食物 / 碎片包 / 被动 / 主动四区，以及独立的删除食物服务区与入口回调。
...
| 棋盘碎片包 | 固定商品卡 | 不动态生成节点；购买胃部碎片后进入 `BoardEdit` 态手动拼贴 |
| 被动道具购买 | 2 个固定道具槽 | 预放 Slot，按槽位索引绑定库存，不动态生成节点 |
| 主动道具购买 | 2 个固定道具槽 | 预放 Slot，按槽位索引绑定库存，不动态生成节点 |
| 删除食物服务 | 固定服务道具卡 | 不动态生成节点；显示动态价格，点击购买后进入菜谱选择，确认目标时扣费并删除 |

库存按「周 + 天」key（`GameRun.BuildShopKey`）在内存 pending 复用：同日再进沿用同一份，不即时存档，离开商店结算时统一存。

### 3.2 菜谱入口

- BattleForm 不再常驻显示固定菜谱卡；统一由左列 `ViewRecipe` 按钮打开中部 `RecipeReadonlyBookView`。
- 商店购买的食物直接加入唯一菜谱，不再选择目标菜谱；购买飞行动画落到左列 `ViewRecipe` 按钮。
- 删除食物等目标选择仍复用中部菜谱页，由 `RecipeBookCoordinator` 区分只读、删除和主动道具选菜流程。
...
同一 `Battle.unity` 世界场景内表现，对应 `BattleWorldController`。中部世界棋盘透出（禁用中部 UI 面板 / 关白底），行动轴按状态配置。

### 5.1 与其它态的差异

- **保留**：左右常驻壳。
- **去掉**：顶部行动轴、商店 / 编辑面板。
- **中部**：世界空间棋盘（`BattleWorldController`）透出；菜谱详情仍通过左列 `ViewRecipe` 进入中部只读页。

### 5.2 交互（`Food`）

- 从世界空间出餐口上菜；`BuildBattleControls` 负责绑定出餐、弃置与拖拽回调。点击 `ViewRecipe` 可查看本局剩余菜品。
...
- 左列 `ViewRecipe` 与 `ViewTable` 分别接入中部只读菜谱页和餐桌预览。
...
- `ApplyView`（在淡出完成后执行）集中做：切换中部面板显隐（`ActionSelect`/`Shop`/`RecipeInspect` 等面板互斥）+ 配置壳（行动轴显隐 / `_backdrop` 白底 / 世界透出）+ 重建内容（行动卡 / 事件卡 / 商店 / 菜谱页 / 世界棋盘）。
...
- 商店购买食物的视觉副本飞向左列 `ViewRecipe` 按钮；动画完成不再刷新固定菜谱计数卡。
