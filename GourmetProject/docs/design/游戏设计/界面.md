...
- **顶部提示**：删除食物至垃圾桶 / 拖拽移动。
- **中部菜谱区**：展示唯一菜谱仓库（`_warehouseContainer` 铺一个 `RecipeWarehouseView`），固定 12 格宽并按可用宽度等比适配；菜品自动紧凑排布，仓库只向下延伸并支持纵向拖动浏览。
...
- 左列分数要求显示**战斗实时值**（`_session.PreviewScore()` / `RequiredScore`）。
