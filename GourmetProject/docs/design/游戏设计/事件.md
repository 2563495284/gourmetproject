...
> 拆分自 [游戏设计总览](../游戏设计.md)。相关：[行动系统](./行动.md)、[时间轴](./时间轴.md)。
>
...
`EffectType` 枚举涵盖即时效果（`GainGold` / `LowerReq` / `GainItem` / `Gamble` / `AddDish` / `UpgradeDish`）与跟进类（`FoodBattle` / `Shop` / `GameOver` / `Victory`）。即时效果由 `EffectResolver` 结算；跟进类为**终止分支**，转成后续动作（经营挑战/商店/结局）。
