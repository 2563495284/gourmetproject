...
`Event` / `Reward` / `Negative` 三种行为的原子行动都是**薄壳**：执行时从事件池里按 `eventType` 分类随机抽一个具体事件再结算。
...
- `Reward` 行动 → 抽 `eventType=Reward` 的事件（即时奖励，通常根页单选项自动结算）。
...
```mermaid
flowchart TD
    roll["RollEvent: 按 eventType/weight/precondition/repeatable 抽事件"] --> enter["进入根页(正文=event.desc, 选项=parentId 为空)"]
...
改 `GameConfig/Datas/event.xlsx`（`event` / `event_option` 两 sheet）后运行 `bash GameConfig/gen.sh` 重新生成 `Config/Gen/*.cs` 与 `StreamingAssets/Config/tbevent*.json`。多效果用「并列 `*` 多行 list」写法（同 `week` 表 `*timelineIds`），单/零效果直接单格。
