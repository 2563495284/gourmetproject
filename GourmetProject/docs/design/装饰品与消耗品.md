...
> 被动表保留目标美味值、食物与碎片隐藏分修正，并以 `itemLuckOffset` 修正装饰品运气；消耗品没有隐藏分。
>
...
配置已按机制物理拆为 `TbPassiveItem` / `TbActiveItem`。装饰品按“运气 → 品质 → 品质内 `baseWeight`”随机，主动表保留 `targetKind` / `targetCount`；消耗品是一个以「目标类型」为核心、跨情境可用的独立子系统。
...
| 随机进度 | 周/天与 `itemLuckOffset` 形成运气，先抽品质再抽单品 | 不配置，抽取只按 `baseWeight` | 两套抽取规则 |
...
结论：**机制维度**（生命周期/持有/触发/效果）两类本质不同，配置和代码都按此分叉；公共字段通过 `ItemDefinition` 统一给运行时消费。
...
调味小票是效果级特例：只在未结算的 Food 经营挑战主界面开放。
...
| `quality` | ✅ | — | 品质只用于装饰品 |
| `archetypeTags` | ✅ | — | 显式玩法流派标签：份数、蛋糕、甜蜜传递；不从名称、描述或 `termId` 推断 |
| `baseWeight` / `specialTags` | ✅ | ✅ | 两表各自配置 |

装饰品品质权重由 `TbItemQualityLuck` 配置，单品表不再配置 `hiddenRange`。公式为 `clamp(constant + baseWeight * exp(luck * luckGrowth), minWeight, maxWeight)`。普通池只使用 Common / Uncommon / Rare / Epic；Negative 仅由显式诅咒入口抽取，Legendary 暂不进入随机。

`slot_boss_passive` 额外使用每周两次星级评鉴保底：第一次生成的 3 个候选若没有任何 `archetypeTags`，第二次先按同一“运气 → 品质 → 单品”规则从可用流派装饰品中抽 1 件，再从剩余普通候选不放回补齐。判断基于候选列表，与玩家最终领取无关；第二次无可用流派候选时正常降级并消费保底，第三次及以后正常随机。
...
`AddFlavor` 选择桌上菜后，同时修改该 `DishInstance` 与其 `SourceDishIndex` 指向的 `RecipeBookSlot`。消耗品只修改内存并消费装饰品和消耗品，不直接落盘；动画不承担数据修改，最终存档遵循既有节点。
...
1. ✅ **配置**：`item.xlsx` 已拆成 `装饰品` / `消耗品` 两张 sheet；`TbPassiveItem` 使用品质、`baseWeight` 与 `itemLuckOffset`，`TbActiveItem` 使用 `targetKind` / `targetCount`；生成 `PassiveItem.cs` / `ActiveItem.cs`。
