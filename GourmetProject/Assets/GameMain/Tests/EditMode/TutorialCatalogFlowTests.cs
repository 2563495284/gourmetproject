...
namespace GourmetProject.Tests.EditMode
{
    public sealed class TutorialCatalogFlowTests
    {
        ...
        public void LegacyResultAndFailureCopy_RemainsAvailableButUsesOldIds()
        {
            ...
            Assert.That(
                win.Steps[0].Message,
                Is.EqualTo("太棒了，老板！这次经营成功，❤️红心不会减少。红心代表餐厅还能承受失败的次数：日常营业和火热营业失败会损失 1 颗，星级评鉴失败会损失 2 颗；红心归零，本局就会结束。"));
            Assert.That(
                loss.Steps[0].Message,
                Is.EqualTo("别灰心，老板！这次没有达到目标，失败会让我们损失❤️红心。日常营业和火热营业失败会损失 1 颗，星级评鉴失败会损失 2 颗；红心归零，本局就会结束。"));
            Assert.That(
