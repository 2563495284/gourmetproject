...
{
    /// <summary>
    /// 结算需要的只读历史 / 菜谱输入。供「大局/小局相同检测」「菜谱检测」等前提读取。
    ...
    public interface IScoreHistory
    {
        ...
        /// <summary>本次美食品鉴（本场战斗）中该 BaseId 已结算过的次数。</summary>
        int MealSettledCount(string baseId);

        /// <summary>本次品鉴菜谱内的全部菜品 BaseId（含已上/未上/在盘）。</summary>
        IReadOnlyList<string> RecipeBaseIds { get; }
    }

    /// <summary>空历史：所有计数为 0，菜谱为空。用于预览或无历史场景。</summary>
