...
namespace GourmetProject.Gameplay.Model
{
    ...
    public sealed class StomachFragmentDef
    {
        ...
        /// 存在格与 <see cref="CellTags"/> 位置用同一公式同步旋转，保持一一对应。id/隐藏分/权重/价格不变。
        /// </summary>
        public StomachFragmentDef Rotated(int times)
        {
            int t = ((times % 4) + 4) % 4;
            StomachFragmentDef current = this;
            for (int i = 0; i < t; i++)
            {
        ...
        private StomachFragmentDef Rotate90()
        {
            ...
            var newTags = new List<CellTag>(CellTags.Count);
            foreach (CellTag ct in CellTags)
            {
                int nx = (h - 1) - ct.Pos.Y;
                int ny = ct.Pos.X;
                newTags.Add(new CellTag(new GridPos(nx, ny), ct.TagId));
            }

            return new StomachFragmentDef(Id, newRows, HiddenMin, HiddenMax, BaseWeight, Price, newTags);
        }
