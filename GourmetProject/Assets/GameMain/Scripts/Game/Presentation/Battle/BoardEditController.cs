...
namespace GourmetProject.Game.Presentation.Battle
{
    ...
    /// 亦承载只读「胃视图」：复用同一套布局但不显示候选托盘、不接受拖拽输入。
    ///
    /// 从 BattleWorldController 拆出，作为其协作组件挂在同一战斗场景根上：
    /// 只持有自身所需的场景引用（棋盘/相机/棋盘格 prefab/菜品根），不再触碰 Food 态私有成员；
    /// 世界互斥态（进入/退出）由 BattleWorldController 外壳调度，本类负责编辑/胃视图的表现与交互。
    /// </summary>
    public sealed class BoardEditController : MonoBehaviour
    {
        ...
        // 编辑页棋盘定位的底部边距：比 Food 态更大，给候选碎片托盘条让位。
        ...
        private BoardCellView _boardCellPrefab;
        private Transform _piecesRoot;
        private Camera _camera;

        // 编辑页自适应单格尺寸与视口半宽/半高（LayoutEditorBoard/ComputeViewport 计算）。
        private float _cellSize = BoardLayout.MaxCellSize;
        private float _halfW = BoardLayout.FallbackHalfW;
        private float _halfH = BoardLayout.FallbackHalfH;
        ...
        /// <summary>由外壳注入共享场景引用（棋盘、棋盘格 prefab、菜品根、相机）。</summary>
        public void Configure(BattleWorldController owner, BoardView boardView, BoardCellView boardCellPrefab, Transform piecesRoot, Camera camera)
        {
            _owner = owner;
        ...
        {
            BoardLayout.ResolveViewport(_camera, out _halfW, out _halfH);
        }
        ...
        private void LayoutEditorBoard(GpBoard board)
        {
            ...
            // 底部边距比 Food 态更大，给候选碎片托盘条让位；其余定位与 Food 态共用 BoardLayout。
            BoardPlacement placement = BoardLayout.Compute(_halfW, _halfH, board, EditBoardBottomMargin);
            ...
            _boardView.Build(board, _cellSize, BoardLayout.Gap, null, _boardCellPrefab);
        ...
        private void BuildDragVisual(StomachFragmentDef def, Vector3 center, Vector3 scale)
        {
            ...
            float avgX = sumX * inv;
            float avgY = sumY * inv;
            float pitch = _cellSize + BoardLayout.Gap;
