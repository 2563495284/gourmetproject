...
{
    /// <summary>设置当前周时间轴已经消耗的天数，便于调试不同时间点的流程。</summary>
    public sealed class DayCommand : ConsoleCommand
    {
        ...
        public override string Description => "设置当前时间轴已消耗的天数。";
        ...
        public override CmdResult Execute(string[] args)
        {
            ...
            {
                // 复用现有周循环：向前跳时补结算跨过的节点，并重建时间轴显示。
                battle.PromptNextAction();
            }

            return CmdResult.Ok(
                $"时间轴已消耗天数：{FormatDays(previousDay)} -> {FormatDays(run.CurrentDay)}。");
        }
