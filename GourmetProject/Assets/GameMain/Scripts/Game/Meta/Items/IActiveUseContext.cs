...
{
    /// <summary>消耗品的使用情境。可用情境由装饰品和消耗品 <c>targetKind</c> 推导（见 docs/design/装饰品和消耗品.md）。</summary>
    ...
    /// <summary>
    /// 消耗品「使用上下文」。经营挑战与局外（商店/行动选择/奖励）各实现一个：
    ...
    public interface IActiveUseContext
    {
        ...
        /// <summary>
        /// 列出当前情境下该装饰品和消耗品的候选目标（供选目标 UI）。
        /// 传入完整定义，使候选校验可感知具体效果语义。
        /// </summary>
        IReadOnlyList<ActiveTarget> EnumerateTargets(ItemDefinition item);

        /// <summary>能力：清空餐桌（仅经营挑战支持）。不支持或无法执行返回 false。</summary>
        bool ClearBoard();

        /// <summary>能力：额外上1 个食物（仅经营挑战支持）。不支持或无法执行返回 false。</summary>
        bool ExtraServe();

        /// <summary>能力：给目标食物永久加分（对标杀戮尖塔2 药水打目标）。不支持或目标无效返回 false。</summary>
        bool AddPermanentScore(ActiveTarget target, float amount);

        /// <summary>能力：复制目标食物。<paramref name="randomKey"/> 供需随机流的情境派生确定性随机。</summary>
        bool DuplicateDish(ActiveTarget target, string randomKey);

        /// <summary>能力：移除目标食物。不支持或目标无效返回 false。</summary>
        bool DestroyDish(ActiveTarget target);

        /// <summary>能力：给目标食物倍率加成（永久倍率）。不支持或目标无效返回 false。</summary>
        ...
        /// <summary>能力：把目标食物的一个风味替换为 <paramref name="toFlavorId"/>。</summary>
        ...
        // —— 排程小票：操作时间轴/Boss，行动选择/商店按能力支持，经营挑战返回 false ——
        ...
        /// <summary>能力：在指定当前或未来整数日追加时间轴节点。</summary>
        bool AddTimelineNode(string actionId, int day);

        /// <summary>能力：在指定当前或未来整数日追加时间轴节点并返回新节点 id。</summary>
